...
function setupGame() {
  ...
  let loadedCount = 0;
  [bgImage, heroImage, enemyFirstRowImage, enemySecondRowImage, enemyThirdRowImage, enemyFourthRowImage].forEach(
    img => {
      img.onload = () => {
        loadedCount++;
        if (loadedCount === 6) {
...
function shootLaser() {
  ...
  const spawnX = hero.x + hero.width / 2 - heroLaserConfig.width / 2 + dx * offset;
  const spawnY = hero.y + hero.height / 2 - heroLaserConfig.height / 2 + dy * offset;
...
function drawGame() {
  ...
  ctx.drawImage(heroImage, -hero.width / 2, -hero.height / 2, hero.width, hero.height);
  ctx.restore();
