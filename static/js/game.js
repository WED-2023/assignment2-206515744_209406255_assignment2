...
function updateEnemies() {
  ...
  if (shouldReverse) {
    ...
    const bottomInDanger = bottomRowEnemies.some(
      (e) => e.y + e.height >= dangerY
    );
...
function tryEnemyShooting() {
  ...
  if (
    enemyLasers.length === 0 ||
    enemyLasers[enemyLasers.length - 1].y > maxEnemyY + spacing
  ) {
    const randomEnemy =
...
function shootLaser() {
  ...
  // Unit direction from hero.rotation (0 = up, +π/2 = right, π = down, …)
  const angle = hero.rotation;
  const dx = Math.sin(angle);
  const dy = -Math.cos(angle);

  // Spawn at the hero’s “nose” (center + half-height in direction)
  const offset = hero.height / 2;
  const spawnX =
    hero.x + hero.width / 2 - heroLaserConfig.width / 2 + dx * offset;
  const spawnY =
    hero.y + hero.height / 2 - heroLaserConfig.height / 2 + dy * offset;

  // Include velocity vector on the laser
  const laser = {
    x: spawnX,
    y: spawnY,
    dx,
...
function handleClick(event) {
  ...
  if (
    !showConfig &&
    isInBox(x, y, canvas.width - 170, canvas.height - 60, 150, 40)
  ) {
  ...
  // Decrease game time
  if (
    isInBox(x, y, startX + 265, startY + 320, 30, 30) &&
    config.gameTime > 120
  ) {
    config.gameTime--;
    draw();
  ...
  // Decrease hero lives
  if (
    isInBox(x, y, startX + 265, startY + 370, 30, 30) &&
    config.heroLives > 3
  ) {
    config.heroLives--;
    draw();
...
function drawGame() {
  ...
  drawText(
    `Timer: ${Math.round(config.gameTime - timeFromGameStart)}`,
    10,
    30,
    "24px Helvetica",
    "rgb(250, 250, 250)"
